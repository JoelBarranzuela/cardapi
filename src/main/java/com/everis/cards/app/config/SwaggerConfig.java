package com.everis.cards.app.config;
...
public class SwaggerConfig {
  ...
  public Docket api() {
    return changeGlobalResponses(new Docket(DocumentationType.SWAGGER_2).select()
        .apis(RequestHandlerSelectors.basePackage("com.everis.app")).paths(PathSelectors.any()).build()
        .apiInfo(metaData()));
  }

  @Bean
  public ApiInfo metaData() {
    return new ApiInfoBuilder().title("API REST CARDS").description("\"API REST LIST CARDS\"")
